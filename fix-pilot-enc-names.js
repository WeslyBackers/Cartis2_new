/**
 * Fix PILOT_ENC product names (Beschrijving)
 * 
 * Updates the 'name' field of PILOT_ENC products to use the OBJNAM
 * value extracted from the description field.
 */
require('dotenv').config({ path: './backend/.env' });
const { Pool } = require('pg');

const pool = new Pool({
  host: process.env.DB_HOST || 'localhost',
  port: process.env.DB_PORT || 5432,
  database: process.env.DB_NAME || 'cartis',
  user: process.env.DB_USER || 'postgres',
  password: process.env.DB_PASSWORD || '',
});

function extractObjnam(description) {
  if (!description) return null;
  const match = description.match(/OBJNAM\s*=\s*(.+?)(?:\n|$)/);
  return match ? match[1].trim() : null;
}

(async () => {
  const client = await pool.connect();
  try {
    // Get all PILOT_ENC products
    const res = await client.query(`
      SELECT p.id, p.code, p.name, p.description 
      FROM products p 
      JOIN production_lines pl ON p.production_line_id = pl.id 
      WHERE pl.code = 'PILOT_ENC' 
      ORDER BY p.code
    `);

    console.log(`Found ${res.rows.length} PILOT_ENC products`);
    console.log('='.repeat(70));

    let updated = 0;
    let skipped = 0;
    let noObjnam = 0;

    await client.query('BEGIN');

    for (const row of res.rows) {
      const objnam = extractObjnam(row.description);
      
      if (!objnam) {
        console.log(`  SKIP (no OBJNAM): ${row.code} - current name: "${row.name}"`);
        noObjnam++;
        continue;
      }

      if (row.name === objnam) {
        skipped++;
        continue;
      }

      console.log(`  UPDATE: ${row.code}: "${row.name}" → "${objnam}"`);
      
      await client.query(
        'UPDATE products SET name = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2',
        [objnam, row.id]
      );
      updated++;
    }

    await client.query('COMMIT');

    console.log('\n' + '='.repeat(70));
    console.log(`Updated: ${updated}`);
    console.log(`Already correct: ${skipped}`);
    console.log(`No OBJNAM found: ${noObjnam}`);
    console.log(`Total: ${res.rows.length}`);
  } catch (err) {
    await client.query('ROLLBACK');
    console.error('Error:', err);
  } finally {
    client.release();
    await pool.end();
  }
})();
